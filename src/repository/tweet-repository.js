...
class TweetRepository{
    ...
    async getWithComments(id){
        try {
            const tweet = await Tweet.findById(id).populate({path:'comments'}).lean();//populate is used to get the comments of the tweet and lean is used to get the plain javascript object
            return tweet;
        }
        catch (error) {
            console.log(error);
